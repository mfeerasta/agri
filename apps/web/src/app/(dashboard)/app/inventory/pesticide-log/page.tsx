...
export default async function PesticideLogPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-4">
      <Masthead section="زرعی دوا لاگ / Pesticide Log" />
      <SectionDivider />
      <InputUsageGrid
        entityId={ctx.entityId}
        data={data}
        cropPlans={plans}
        title="Pesticide Log"
        titleUr="زرعی دوا لاگ"
        recordLabel="Record pesticide"
        recordLabelUr="زرعی دوا درج کریں"
        itemLabel="pesticide"
        itemLabelUr="زرعی دوا"
        xlsxRoutePrefix="/api/inventory/pesticide-log/xlsx"
        revalidatePath="/inventory/pesticide-log"
      />
    </div>
  );
}
