 ...
 * POST /api/ocr/repair
 *
 * Same shape as /api/ocr/diesel but extracts repair-quote fields from a
 * handwritten Urdu workshop slip. Rate-limited at 10 req/user/min.
 */
import { NextResponse } from 'next/server';
import { extractRepairQuote } from '@zameen/shared';
...
export async function POST(req: Request): Promise<Response> {
  ...
  const bucket = rateLimit(`ocr:repair:${userData.user.id}`, 10, 60_000);
  if (!bucket.allowed) {
  ...
  const extract = await extractRepairQuote(imageUrl);
