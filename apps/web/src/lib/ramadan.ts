// ramadan
// Server helpers for Ramadan-aware UI flows. Reads Aladhan for sehri/iftar in
// Lahore. Cached in-process for 6 hours since these values shift by <1 minute
// per day. Used by Field PWA banner, scheduler heuristics, and the digest
// dispatcher.
...
async function getTimings(date: Date): Promise<CachedTimings | null> {
  ...
  try {
    ...
    const entry: CachedTimings = {
      ...
      sehriLocal: body.data.timings.Imsak ?? body.data.timings.Fajr ?? '',
...
export async function isRamadanToday(): Promise<boolean> {
...
function combineLocalTimeWithPktDate(localHHmm: string, base: Date): Date | null {
  const match = /^(\d{2}):(\d{2})/.exec(localHHmm);
  if (!match) return null;
  const key = pktDateKey(base);
  const iso = `${key}T${match[1]}:${match[2]}:00+05:00`;
  return new Date(iso);
}
...
export async function getIftarTime(date: Date = new Date()): Promise<Date | null> {
  ...
  return combineLocalTimeWithPktDate(t.iftarLocal, date);
}

export async function getSehriTime(date: Date = new Date()): Promise<Date | null> {
  const t = await getTimings(date);
  if (!t || !t.sehriLocal) return null;
  return combineLocalTimeWithPktDate(t.sehriLocal, date);
}
